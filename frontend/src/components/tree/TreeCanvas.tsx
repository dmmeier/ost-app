  ...
  const rfNodes: RFNode[] = visibleNodes.map((node) => {
    ...
    return {
      ...
      data: {
        ...
        hasAssumption: !!(node.assumption || "").trim() || (node.assumptions || []).length > 0,
        assumptionCount: (node.assumptions || []).length,
        confirmedAssumptionCount: (node.assumptions || []).filter((a: any) => a.status === "confirmed").length,
        rejectedAssumptionCount: (node.assumptions || []).filter((a: any) => a.status === "rejected").length,
        tags: node.tags || [],
