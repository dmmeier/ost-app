  ...
  const rfNodes: RFNode[] = visibleNodes.map((node) => {
    ...
    return {
      ...
      data: {
        ...
        hasAssumption: !!(node.assumption || "").trim() || (node.assumptions || []).some((a: any) => (a.text || "").trim()),
        assumptionCount: (node.assumptions || []).filter((a: any) => (a.text || "").trim()).length,
        confirmedAssumptionCount: (node.assumptions || []).filter((a: any) => a.status === "confirmed" && (a.text || "").trim()).length,
        rejectedAssumptionCount: (node.assumptions || []).filter((a: any) => a.status === "rejected" && (a.text || "").trim()).length,
        tags: node.tags || [],
